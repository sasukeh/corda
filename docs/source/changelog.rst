...
* API changes:
    ...
    * ``PluginServiceHub.registerFlowInitiator`` has been deprecated and replaced by ``registerServiceFlow`` with the
      marker Class restricted to ``FlowLogic``. Inline with the introduction of ``InitiatingFlow``, it throws an
    ...
    * Also related to ``InitiatingFlow``, the ``shareParentSessions`` boolean parameter of ``FlowLogic.subFlow`` has been
      removed. It's purpose was to allow subflows to be inlined with the parent flow - i.e. the subflow does not initiate
      ...
      other side, requires the annotation.
